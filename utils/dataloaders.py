...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True, transforms=None):
        ...
        self.stride = stride
        ###########
        self.stop_event = threading.Event()  # 종료 플래그 추가
    ...
    def update(self, i, cap, stream):
        ...
        n, f, read = 0, self.frames[i], 1  # frame number, frame array, inference every 'read' frame
        ################
        fps = cap.get(cv2.CAP_PROP_FPS)  # Get the stream's FPS
        if fps <= 0:  # Default to 30 FPS if unable to retrieve
            fps = 20
        frame_delay = 1.0 / fps  # Time delay between frames in seconds

        while True:  # Main loop
            if self.stop_event.is_set():  # Check if stop event is set
                print(f"Stop event received for stream {i}, terminating update thread.")
                break

            if not cap.isOpened():  # If the stream is not open, terminate the thread
                print(f"Stream {i} is not open, terminating update thread.")
                break

            start_time = time.time()  # Record the start time for the frame
            n += 1
            cap.grab()
            if n % read == 0:
                success, im = cap.retrieve()
                ...
                #!!!!!!!!!!!!!!!!!!!!!
                # print(f"Stream update thread 진행중--------!")
        ...
        cap.release()  # Stop event triggered, release resources
        print(f"Stream {i} update thread terminated.")

    def stop(self):
        """Set stop event to terminate threads and release resources."""
        ...
        for i in range(len(self.sources)):
            self.frames[i] = 0  # Reset frame counters
            self.imgs[i] = None  # Clear images

        print("All stream threads have been stopped and resources released.")
